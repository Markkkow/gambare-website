...
async function getAvailability(request) {
  try {
    ...
    const rows = await supabaseRest(`bookings?date=eq.${encodeURIComponent(date)}&select=tables`);
    const booked = (rows || []).reduce((sum, b) => sum + (Number(b.tables) || 0), 0);

    return json({ success: true, date, total: TOTAL_TABLES, booked, available: Math.max(0, TOTAL_TABLES - booked) });
  } catch (err) {
    console.error(err);
...
export default { fetch: getAvailability };
